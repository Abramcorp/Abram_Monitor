  ...
  return state.knowledge
    ...
    .map((bank) => ({
      ...
      programs: (bank.programs || []).filter((program) => {
        ...
        return [bank.bank, program.program, program.programType, program.category, program.amountRange, program.notes, requirementText].join(" ").toLowerCase().includes(query);
      })
    }))
    .filter((bank) => {
