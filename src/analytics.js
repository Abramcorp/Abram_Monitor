...
function normalizeDeal(raw = {}) {
  ...
  return {
    ...
    programTermRange: cleanText(raw.programTermRange || raw.termRange || raw.term),
    stage,
    ...
    campaignId: cleanText(raw.campaignId),
    wave: Math.max(0, Math.trunc(toNumber(raw.wave))),
    entryType: cleanText(raw.entryType) || "application",
    decisionType: cleanText(raw.decisionType),
    validUntil: toIsoDate(raw.validUntil),
    conditions: Array.isArray(raw.conditions) ? raw.conditions.map(cleanText).filter(Boolean) : [],
    refusalReasonCode: cleanText(raw.refusalReasonCode),
    integrationSource: cleanText(raw.integrationSource),
    integrationIdempotencyKeyHash: cleanText(raw.integrationIdempotencyKeyHash),
    integrationRequestHash: cleanText(raw.integrationRequestHash),
...
function toApplicationSummary(deal) {
  return {
    ...
    programTermRange: deal.programTermRange,
    stage: deal.stage,
